...
#include "config/core.h" // GP_BOUND_ITEMS, MAX_SPIRITBALL, SECURE_NPCTIMEOUT
#include "pc.h"
...
int pc_isUseitem(struct map_session_data *sd, int n)
{
	...
	while (1) {
		// Normal classes (no upper, no baby)
		if (item->class_upper&ITEMUPPER_NORMAL && !(sd->class_&(JOBL_UPPER | JOBL_BABY)))
			break;
		// pre-re has no use for the extra, so we maintain the previous for backwards compatibility
		if (item->class_upper&ITEMUPPER_UPPER && sd->class_&(JOBL_UPPER))
			break;
		// Baby classes (no third classes)
		if (item->class_upper&ITEMUPPER_BABY && sd->class_&JOBL_BABY)
			break;
		return 0;
	}
